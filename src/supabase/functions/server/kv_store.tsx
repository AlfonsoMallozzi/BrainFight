...
// View at https://supabase.com/dashboard/project/dyiuhcitobahtapumdmb/database/tables
...
export const get = async (key: string): Promise<any> => {
  const supabase = client()
  const { data, error } = await supabase.from("kv_store_83bde5a9").select("value").eq("key", key).maybeSingle();
  if (error) {
    throw new Error(error.message);
  }
  return data?.value;
...
export const mget = async (keys: string[]): Promise<any[]> => {
  const supabase = client()
  const { data, error } = await supabase.from("kv_store_83bde5a9").select("value").in("key", keys);
  if (error) {
    throw new Error(error.message);
  }
